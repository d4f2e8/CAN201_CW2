...
remote_current_working_directory = '/home/tc/data/cw1/'
    ...
    try:
        ...
        for f in py_files:
            ...
            if len(components) > 1:  # Files in folders
                target_dir = join(remote_current_working_directory, '/'.join(components[:-1]))
                ssh.exec_command(f'mkdir -p {target_dir}')
            else:  # Files in CWD
                target_dir = remote_current_working_directory
            print(f'Send {components[-1]} to {remote_ip}:{target_dir}')
            sftp.put(f, join(target_dir, components[-1]))

        if len(py_files) > 0:
            # the first file will be the main file
            main_py = join(remote_current_working_directory, py_files[0])
            print(f'###################### RUN {py_files[0]} ######################')
            # execute the code
            stdin, stdout, stderr = ssh.exec_command(f'{remote_python_interpreter} {main_py} --ip ',
